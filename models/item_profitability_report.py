...
class ItemProfitabilityReport(models.Model):
    _name = 'havano.item.profitability.report'
        ...
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW %s AS (
                SELECT
                    ...
                    SUM(CASE WHEN m.move_type = 'out_refund' THEN -l.quantity * COALESCE((p.standard_price->>m.company_id::text)::numeric, 0) ELSE l.quantity * COALESCE((p.standard_price->>m.company_id::text)::numeric, 0) END) as total_buy_price,
                ...
                GROUP BY
                    l.product_id, t.categ_id, m.invoice_date, l.currency_id
            )
        """ % (self._table,))
